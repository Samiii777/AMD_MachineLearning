...
def main():
    ...
    logging.info(f"Average eval_count: {avg_eval_count:.2f}")
    logging.info(f"Average eval_duration: {avg_eval_duration:.2f} ms")

    if avg_eval_duration != 0:
        avg_performance = avg_eval_count / avg_eval_duration * 1000
        logging.info(f"Average Performance: {avg_performance:.2f}(tokens/s)")
    else:
        logging.info("Average eval_duration is zero. Cannot calculate average performance.")
